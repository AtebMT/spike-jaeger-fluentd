// Nicked from https://github.com/chunpu/syslogd and modified to actually work
...
function parser(msg, rinfo) {
    ...
    var bits = msg.split(' ')
    var pri = bits.shift()
    var timestamp = bits.shift()
    var hostname = bits.shift()
    var appname = bits.shift()
    var pid = bits.shift()
    var msgid = bits.shift()
    var data = bits.shift()
    var msg = bits.join(' ')
    var priIndex = pri.indexOf('>')
    var pri = pri.substr(1, priIndex - 1)
    pri = parsePRI(pri)
